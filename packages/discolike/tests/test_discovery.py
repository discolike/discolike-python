...
def test_discover_builds_query_and_parses_list(make_client: ClientFactory) -> None:
    ...
    with make_client(handler) as client:
        results = client.discover(DiscoverParams(icp_prompt="B2B fintech in DE", country=["DE", "AT"], max_records=50))
    ...
    assert "offset" not in dict(seen["params"])
...
def test_discover_passes_unknown_fields_through(make_client: ClientFactory) -> None:
    seen = {}
    ...
    with make_client(handler) as client:
        client.discover(DiscoverParams.model_validate({"future_flag": "on"}))

    assert seen["params"]["future_flag"] == "on"


def test_discover_rejects_out_of_range_similarity_before_any_request() -> None:
...
def test_count(make_client: ClientFactory) -> None:
    def handler(request: httpx2.Request) -> httpx2.Response:
        ...
        assert httpx2.QueryParams(request.url.query)["category"] == "CYBERSECURITY"
        return httpx2.Response(200, json={"count": 1234})

    with make_client(handler) as client:
        result = client.count(CountParams(category=["CYBERSECURITY"]))
...
async def test_discover_async(make_async_client: AsyncClientFactory) -> None:
    ...
    async with make_async_client(handler) as client:
        results = await client.discover(DiscoverParams(domain=["stripe.com"]))
    assert results[0].domain == "a.com"


async def test_count_async(make_async_client: AsyncClientFactory) -> None:
    def handler(request: httpx2.Request) -> httpx2.Response:
        return httpx2.Response(200, json={"count": 7})

    async with make_async_client(handler) as client:
        result = await client.count(CountParams(country=["US"]))
