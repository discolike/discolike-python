...
class EmailResource(SyncAPIResource):
    @api_route("POST", "/email/find", openapi=False)
    def find(self, *, first_name: str, last_name: str, domain: str) -> EmailJob:
    ...
    @api_route("POST", "/email/find/batch", openapi=False, ignore_params=("contacts",))
...
class AsyncEmailResource(AsyncAPIResource):
    @api_route("POST", "/email/find", openapi=False)
    async def find(self, *, first_name: str, last_name: str, domain: str) -> AsyncEmailJob:
        body = {k: v for k, v in locals().items() if k != "self"}
