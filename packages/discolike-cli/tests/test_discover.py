...
def test_discover_param_removed_kwarg_exits_2(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_build_client(monkeypatch, _discover_ok)
...
def test_discover_negate_option_forwarded(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, httpx.QueryParams] = {}
...
def test_discover_icp_text_option_removed(monkeypatch: pytest.MonkeyPatch) -> None:
    _install_build_client(monkeypatch, _discover_ok)
