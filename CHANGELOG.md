...
- SDK: discover and count take `sub_industry` / `negate_sub_industry` (217 second-level labels scoped to their parent industry category; a bare label like `ROOFING` adds `CONSTRUCTION` to `category` server-side) and `lat` / `lon` / `radius` (`50km`, `30mi`, or a bare number meaning kilometres; defaults to 50km) or `bbox` (`min_lat,min_lon,max_lat,max_lon`, mutually exclusive with `lat`/`lon`/`radius`, longitudes may wrap the antimeridian). The SDK applies the platform's own bbox rules before the request leaves the client: four finite numbers, `-90 <= min_lat < max_lat <= 90`, longitudes within ±180 and `min_lon` different from `max_lon`.
...
- CLI: `discover` and `count` gain `--sub-industry`, `--negate-sub-industry`, `--lat`, `--lon`, `--radius`, `--bbox`.
