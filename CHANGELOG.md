...
Folds in the never-published 0.3.0 changes below.
...
- CLI: `discolike auth login` remembers the OAuth client it registered (`oauth_client` in the config file) and reuses it on the next login for the same server, so the browser consent screen is only asked once per machine. `auth logout` forgets it.
...
### Request models (previously staged as 0.3.0)
